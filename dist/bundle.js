 ...
 */
var Read2MePolyfills = function () {
    function Read2MePolyfills() {
        _classCallCheck(this, Read2MePolyfills);
    }

    _createClass(Read2MePolyfills, null, [{
        key: 'forEach',
    ...
    _createClass(Read2MeWidgetPlayer, [{
        ...
        value: function handleAutoplay() {
            ...
            this.audioController.audio.addEventListener('canplay', function () {
                _this.audioController.play();
                _this.displayPauseButton();
            });
        }
    }, {
        key: 'setTheme',
        ...
        value: function handlePlayback() {
            ...
            this.audioController.audio.addEventListener('playing', function () {
                _this2.hideLoader();
            });

            this.playbackContainer.addEventListener('click', function () {
                if (_this2.isPlayButtonShown()) {
                    _this2.audioController.play();
                    _this2.displayLoader();
                    _this2.displayPauseButton();
                } else if (_this2.isPauseButtonShown()) {
                    _this2.audioController.pause();
                    _this2.displayPlayButton();
                } else {
                    _this2.audioController.replay();
                    _this2.displayPauseButton();
                }
            });

            this.audioController.audio.addEventListener('ended', function () {
                _this2.displayReplayButton();
            });

            this.audioController.audio.addEventListener('stalled', function () {
                _this2.displayLoader();
            });
        }
    }, {
        ...
        value: function handleQuickControls() {
            ...
            rewind.addEventListener('click', function () {
                _this3.audioController.rewindForXSeconds(10);

                if (_this3.isReplayButtonShown()) _this3.displayPlayButton();
            });

            forward.addEventListener('click', function () {
                _this3.audioController.forwardForXSeconds(10);
            });
        }
    }, {
        key: 'handleScrubber',
        value: function handleScrubber() {
            ...
            this.scrubber.on('slideStart', function () {
                _this4.isScrubberBeingDragged = true;
            });

            this.scrubber.on('slideStop', function (newCurrentTime) {
                _this4.isScrubberBeingDragged = false;
                _this4.audioController.setCurrentTime(newCurrentTime);

                if (_this4.isReplayButtonShown() && newCurrentTime !== _this4.audioController.getDuration()) _this4.displayPlayButton();
            });

            this.audioController.audio.addEventListener('timeupdate', function () {
                if (_this4.isScrubberBeingDragged) return false;

                _this4.scrubber.setValue(Math.round(_this4.audioController.getCurrentTime()));
            });
        }
    }, {
        ...
        value: function handleSpeakingRateChange() {
            ...
            this.speakingRate.on('change', function (values) {
                _this5.audioController.setPlaySpeed(values.newValue);
            });
        }
    }, {
        key: 'displayLoader',
        ...
        value: function handleViewportResize() {
            ...
            window.addEventListener('resize', function () {
                _this6.scalePlayerDownOnSmallScreens();
            });
        }
    }]);
...
var Read2MePlayerBuilder = function () {
    ...
    _createClass(Read2MePlayerBuilder, [{
        ...
        value: function _replaceBlueprintWithPlayer(elem) {
            ...
            var appId = parseInt(elem.getAttribute('data-app-id'));
            ...
            this._makeApiCalls(backendWrapper, function (responseResult) {
                // success
                _this2.playerInstances[playerId].finishInitialisation(new Read2MeAudioController(responseResult.audio_url), responseResult);
            }, function (response) {
                _this2.playerInstances[playerId].hideLoader();
...
Read2MeHelpers.documentReady(function () {
    if (Read2MeHelpers.isIe()) return false;
